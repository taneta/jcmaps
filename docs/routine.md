...
| A suggestion arrives in the Google Form | Read it within a week. A source with a public calendar becomes a Change ticket. An event you approve goes into `data/approved_events.json` (title, start and end in Jersey City time, place, address, link, price, kids, a short description, and an organization's name, never a person's); it shows after the next run and drops off by itself when it is over. Reply if they left an email. Their email stays in the form's responses, never in the repo. |
