...
3. **Geocode.** Venue address to coordinates, cached in the repo. Nominatim at one request per second with a User-Agent, or Photon. Library branches are seeded once from OpenStreetMap. An event that fails to geocode keeps its record and is shown in the list without a pin.
...
5. **Check.** Rules only in v1: required fields, start in the future or ongoing, inside the boundary, duplicates. Duplicates share a date, start within 30 minutes, sit within 150 m and have similar titles; the record with more evidence wins and both URLs are kept. A title containing cancelled or postponed sets the status to cancelled. Failures go to the run report with a reason.
