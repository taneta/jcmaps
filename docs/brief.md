...
3. **Geocode.** Venue address to coordinates, cached in the repo. One venue per address: records with the same house number, street and city share it however each source spells them, and the other names become aliases. Nominatim at one request per second with a User-Agent, or Photon. Known venues (the library branches, seeded once from OpenStreetMap, and any venue a person corrects) are pinned from `data/venue_points.json` and never geocoded; a street answer counts as no answer. A corner is placed at the node its two streets share in OpenStreetMap, and a venue the address could not place gets one try with the model's reading of the listing (a landmark, quoted from the text), which the map service must confirm. An event that fails to geocode keeps its record and is shown in the list without a pin, and the run report lists such venues with their event counts.
